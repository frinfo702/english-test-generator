...
function loadJson(file: string): LectureData {
  const raw = fs.readFileSync(path.join(QUESTION_DIR, file), "utf-8");
  return JSON.parse(raw) as LectureData;
}

describe("TOEFL Listening: Academic Talk (Lecture) JSON structure", () => {
  ...
  // Each lecture should have 4 questions (schema: 1講義あたり4問)
  it.each(files)("%s has 4 questions", (file) => {
    const data = loadJson(file);
  ...
  // Audio should have Lecturer role
  it.each(files)("%s audio segments have Lecturer role", (file) => {
    const data = loadJson(file);
  ...
  // At least 2 different question types per lecture
  ...
  // At least 3 different types appear across all files
  it("at least 3 valid question types appear across files", () => {
